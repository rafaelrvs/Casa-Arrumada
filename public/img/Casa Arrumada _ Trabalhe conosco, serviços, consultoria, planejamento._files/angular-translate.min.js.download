...
var C=function(I){var G=[],D=angular.lowercase(I),F=0,J=m.length;
for(;
F<J;
F++){G.push(angular.lowercase(m[F]))
}if(G.indexOf(D)>-1){return D
}if(r){if(r[I]){var E=r[I];
if(G.indexOf(angular.lowercase(E))>-1){return E
}}}var H=I.split("_");
if(H.length>1&&G.indexOf(angular.lowercase(H[0]))>1){return H[0]
}};
...
this.$get=["$log","$injector","$rootScope","$q",function(Q,T,L,Y){var ac,J=T.get(u||"$translateDefaultInterpolation"),ab=false,G={},M={},E,O;
var R=function(ae,ag,ah){var af=Y.defer();
ae=ae.trim();
var ai=function(){var ak=y?M[y]:M[f];
E=0;
if(k&&!ak){var al=ac.get(h);
ak=M[al];
if(s&&s.length){var aj=D(s,al);
E=aj>-1?aj+=1:0;
s.push(y)
}}return ak
}();
if(!ai){ad(ae,ag,ah).then(af.resolve,af.reject)
}else{ai.then(function(){ad(ae,ag,ah).then(af.resolve,af.reject)
},af.reject)
}return af.promise
};
var D=function(ah,af){for(var ag=0,ae=ah.length;
ag<ae;
ag++){if(ah[ag]===af){return ag
}}return -1
};
var N=function(ae){if(q){ae=[q,ae].join(" ")
}if(l){ae=[ae,l].join(" ")
}return ae
};
var W=function(ae){f=ae;
L.$emit("$translateChangeSuccess");
if(k){ac.set(R.storageKey(),f)
}J.setLocale(f);
angular.forEach(G,function(af,ag){G[ag].setLocale(f)
});
L.$emit("$translateChangeEnd")
};
var Z=function(af){if(!af){throw"No language key specified for loading."
}var ae=Y.defer();
L.$emit("$translateLoadingStart");
ab=true;
T.get(z)(angular.extend(w,{key:af})).then(function(ah){var ag={};
L.$emit("$translateLoadingSuccess");
if(angular.isArray(ah)){angular.forEach(ah,function(ai){angular.extend(ag,b(ai))
})
}else{angular.extend(ag,b(ah))
}ab=false;
ae.resolve({key:af,table:ag});
L.$emit("$translateLoadingEnd")
},function(ag){L.$emit("$translateLoadingError");
ae.reject(ag);
L.$emit("$translateLoadingEnd")
});
return ae.promise
...
if(k){ac=T.get(k);
if(!ac.get||!ac.set){throw new Error("Couldn't use storage '"+k+"', missing get() or set() method!")
}}if(angular.isFunction(J.useSanitizeValueStrategy)){J.useSanitizeValueStrategy(x)
}if(o.length){angular.forEach(o,function(ae){var af=T.get(ae);
af.setLocale(y||f);
if(angular.isFunction(af.useSanitizeValueStrategy)){af.useSanitizeValueStrategy(x)
}G[af.getInterpolationIdentifier()]=af
})
}var S=function(af){var ae=Y.defer();
if(j.hasOwnProperty(af)){ae.resolve(j[af]);
return ae.promise
}else{M[af].then(function(ag){g(ag.key,ag.table);
ae.resolve(ag.table)
},ae.reject)
}return ae.promise
};
var H=function(ai,af,ah,ae){var ag=Y.defer();
S(ai).then(function(aj){if(aj.hasOwnProperty(af)){ae.setLocale(ai);
ag.resolve(ae.interpolate(aj[af],ah));
ae.setLocale(f)
}else{ag.reject()
}},ag.reject);
return ag.promise
};
var aa=function(aj,ag,ai,af){var ae,ah=j[aj];
if(ah.hasOwnProperty(ag)){af.setLocale(aj);
ae=af.interpolate(ah[ag],ai);
af.setLocale(f)
}return ae
};
var K=function(ai,af,ah,ae){var ag=Y.defer();
if(ai<s.length){var aj=s[ai];
H(aj,af,ah,ae).then(function(ak){ag.resolve(ak)
},function(){var ak=K(ai+1,af,ah,ae);
ag.resolve(ak)
})
}else{ag.resolve(af)
}return ag.promise
};
var I=function(ai,ag,ah,af){var ae;
if(ai<s.length){var aj=s[ai];
ae=aa(aj,ag,ah,af);
if(!ae){ae=I(ai+1,ag,ah,af)
}}return ae
};
var P=function(af,ag,ae){return K(O>0?O:E,af,ag,ae)
};
var F=function(af,ag,ae){return I(O>0?O:E,af,ag,ae)
};
var ad=function(af,ah,aj){var ag=Y.defer();
var ai=f?j[f]:j,ae=aj?G[aj]:J;
if(ai&&ai.hasOwnProperty(af)){var ak=ai[af];
if(ak.substr(0,2)==="@:"){R(ak.substr(2),ah,aj).then(ag.resolve,ag.reject)
}else{ag.resolve(ae.interpolate(ak,ah))
}}else{if(n&&!ab){T.get(n)(af,f)
}if(f&&s&&s.length){P(af,ah,ae).then(function(al){ag.resolve(al)
},function(al){ag.reject(N(al))
})
}else{ag.reject(N(af))
}}return ag.promise
};
var V=function(ag,ah,aj){var af,ai=f?j[f]:j,ae=aj?G[aj]:J;
if(ai&&ai.hasOwnProperty(ag)){var ak=ai[ag];
if(ak.substr(0,2)==="@:"){af=V(ak.substr(2),ah,aj)
}else{af=ae.interpolate(ak,ah)
}}else{if(n&&!ab){T.get(n)(ag,f)
}if(f&&s&&s.length){E=0;
af=F(ag,ah,ae)
}else{af=N(ag)
}}return af
...
R.fallbackLanguage=function(ag){if(ag!==undefined&&ag!==null){c(ag);
if(z){if(s&&s.length){for(var af=0,ae=s.length;
af<ae;
af++){if(!M[s[af]]){M[s[af]]=Z(s[af])
}}}}R.use(R.use())
}if(p){return s[0]
}else{return s
}};
R.useFallbackLanguage=function(af){if(af!==undefined&&af!==null){if(!af){O=0
}else{var ae=D(s,af);
if(ae>-1){O=ae
}}}};
R.proposedLanguage=function(){return B
};
R.storage=function(){return ac
};
R.use=function(af){if(!af){return f
}var ae=Y.defer();
L.$emit("$translateChangeStart");
if(!j[af]&&z){B=af;
M[af]=Z(af).then(function(ag){g(ag.key,ag.table);
ae.resolve(ag.key);
if(B===af){W(ag.key);
B=undefined
}},function(ag){B=undefined;
...
R.refresh=function(ak){if(!z){throw new Error("Couldn't refresh translation table, no loader registered!")
}var af=Y.defer();
function aj(){af.resolve();
L.$emit("$translateRefreshEnd")
}function ai(){af.reject();
L.$emit("$translateRefreshEnd")
}L.$emit("$translateRefreshStart");
if(!ak){var ah=[];
if(s&&s.length){for(var ag=0,ae=s.length;
ag<ae;
ag++){ah.push(Z(s[ag]))
}}if(f){ah.push(Z(f))
}Y.all(ah).then(function(al){angular.forEach(al,function(am){if(j[am.key]){delete j[am.key]
}g(am.key,am.table)
});
if(f){W(f)
}aj()
})
}else{if(j[ak]){Z(ak).then(function(al){g(al.key,al.table);
if(ak===f){W(f)
}aj()
},ai)
}else{ai()
...
R.instant=function(af,ag,aj){if(typeof af==="undefined"||af===""){return af
}af=af.trim();
var ae,ak=[];
if(y){ak.push(y)
}if(f){ak.push(f)
}if(s&&s.length){ak=ak.concat(s)
}for(var ah=0,al=ak.length;
ah<al;
ah++){var ai=ak[ah];
if(j[ai]){if(j[ai][af]){ae=V(af,ag,aj)
}}if(typeof ae!=="undefined"){break
}}if(!ae){ae=af;
if(n&&!ab){T.get(n)(af,f)
}}return ae
};
if(z){if(angular.equals(j,{})){R.use(R.use())
}if(s&&s.length){for(var U=0,X=s.length;
U<X;
U++){M[s[U]]=Z(s[U])
}}}return R
}]
...
}if(g){var r=function(u){o.$observe(u,function(v){s.interpolateParams[angular.lowercase(u.substr(14))]=v
})
};
for(var n in o){if(o.hasOwnProperty(n)&&n.substr(0,14)==="translateValue"&&n!=="translateValues"){r(n)
}}}var m=function(w,u){t.html(w);
...
var p=function(){if(!i&&!g){return function(){var u=s.$watch("translationId",function(v){if(s.translationId&&v){e(v,{},k).then(function(w){m(w,s);
u()
},function(w){m(w,s);
u()
})
...
}else{return function(){s.$watch("interpolateParams",function(u){if(s.translationId&&u){e(s.translationId,u,k).then(function(v){m(v,s)
},function(v){m(v,s)
})
}},true)
}
...
angular.module("pascalprecht.translate").filter("translate",["$parse","$translate",function(b,a){return function(c,e,d){if(!angular.isObject(e)){e=b(e)()
}return a.instant(c,e,d)
}
}]);